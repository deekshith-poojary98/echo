...
Current tagged version is **v0.9.7**. **0.9.8** (union narrowing in `if type(...)`) continues the **0.9.6–0.9.9** stdlib/language tail. **0.8 OOP spine is complete** (0.8.0–0.8.9). Failure model stays abort + `*Or`.
...
**Status: 0.9.0–0.9.8 implemented; 0.9.9 drafted.**
...
Do not move global holds into 0.9 polish without an explicit series start. **0.8.0–0.8.9** and **0.9.0–0.9.8** are implemented; **0.9.9** is drafted. The closed 0.7 spine is in the **0.7.x** table.

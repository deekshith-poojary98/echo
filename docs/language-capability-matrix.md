...
**Aligned through:** v0.8.9 (first PyPI release of `echolang`). Earlier rows for field defaults, `new { ... }`, interfaces, methods/`this`, nominal classes, param `const`, hash rest/rename, `switch`, unions, const, destructure, exact, builtins-as-values, range-as-value, modules, REPL, fmt/lint/test, `*Or`, map/filter stay marked as shipped.
...
| Classes / OOP | Runtime | Supported (0.8.9; no inheritance) | Active | 0.8.x |
...
**Limitations.** `format` has no width / precision / alignment specs. Regex is thin (`regexMatch` / `regexFind` / `regexReplace` / `regexSplit`); no regex object type.
...
| Classes / OOP | Supported (0.8.9) | Classes + `new { }` fields/defaults + bound/unbound/type methods + optional `implements` + interfaces; no inheritance |
...
| Classes | Supported (0.8.9; no inheritance) |
...
4. Docs generator or playground HTTP policy (drafted **1.1.5**; error-code docs shipped in **1.1.4**)
...
added syntax and tooling; see `CHANGELOG.md` through **0.8.9**.

...
**Aligned through:** v0.8.3 (`new { ... }` field block). Earlier rows for interfaces, methods/`this`, nominal classes, param `const`, hash rest/rename, `switch`, unions, const, destructure, exact, builtins-as-values, range-as-value, modules, REPL, fmt/lint/test, `*Or`, map/filter stay marked as shipped.
...
| Bucket | Verdict |
| --- | --- |
| Language fundamentals | Supported through 0.8.3 surface |
...
| Classes / OOP | Runtime | Supported (0.8.3; no inheritance) | Active | 0.8.x |
...
| Classes / OOP | Supported (0.8.3) | Classes + `new { }` fields + methods + interfaces; no inheritance |
...
| Classes | Supported (0.8.3; no inheritance) |
...
destructuring, exact objects, unions, range-as-value, nominal classes (0.8.0), methods + `this` (0.8.1), interfaces (0.8.2), `new { ... }` fields (0.8.3).
...
added syntax and tooling; see `CHANGELOG.md` through **0.8.3**.

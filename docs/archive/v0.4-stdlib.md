...
> **Historical archive.** Contract for the v0.4 builtins only. See [Built-in Methods](/standard-library/built-in-methods) for the current catalog.
>
> **Status:** Contract for the v0.4 builtins (plus later host/tooling notes below). Implemented.
> **0.4.1–0.4.3** add more host builtins and `echo check`. **0.5.0** adds language-basics builtins plus multiline strings and `.5` / scientific literals. **0.5.3** adds `*Or` twins. Tooling after that: see `CHANGELOG.md`.
> **Boundary:** [v0.4 language vs stdlib](/archive/v0.4-language-vs-stdlib)

...
- Nominal `class` + construction (`Point { x: 3, y: 4 }`) (0.8.0); methods + `this` (0.8.1)
- No control-flow narrowing via `if type(x) == "..."` yet (type arms on `switch` cover the main case)

Still out of scope. Interfaces, `try`/`catch`, and the other frozen holds are not implemented.

## Current Limitations
- No `interface` (0.8.2); no inheritance
- No generics

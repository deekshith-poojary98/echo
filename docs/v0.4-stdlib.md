...
> **Status:** Implemented language/stdlib contract for v0.4; 0.4.1 adds more builtins and `echo check`
...
- A host may deny files (playground). `readFile`, `writeFile`, and `fileExists` then abort. That is an Echo runtime error, not a silent no-op or `false`.

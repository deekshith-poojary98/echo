...
> **Status:** Implemented language/stdlib contract for v0.4; 0.4.1–0.4.3 add more host/stdlib builtins and `echo check`. **0.5.0** adds language-basics builtins plus multiline strings and `.5` / scientific number literals.
...
- A host may deny files (playground). `readFile`, `writeFile`, `fileExists`, `isDir`, `listFiles`, `mkdir`, `removeFile`, and `copyFile` then abort. That is an Echo runtime error, not a silent no-op or `false`. `cwd`, `exit`, and `pathJoin` stay available.
...
formatter, VM, generics, classes, async.

...
`echo` with no source file starts a REPL. Each line is a complete program
(no persistent bindings). `--plain` is supported. `exit(code)` leaves the REPL
with that process code.

`echo test path.echo` runs that file. Exit 0 is pass. There is no test DSL.

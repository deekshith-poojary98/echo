...
`echo test path.echo` runs that file, including imported modules. Exit 0 is pass.
`exit(2)` is a failed test. `echo test` with no path prints help and exits 2.
The first argv token `test` is the subcommand; `echo test.echo` still runs a
file named `test.echo`.
